...
try {
    ...
    if ($currentBranch -eq $branch) {
        if ((git -C $wt rev-parse HEAD).Trim() -ne $expectedMain) {
            throw "STOP: existing completion branch is not at the audited main commit. Do not reset it automatically."
        }
        if ($remoteBranch.Count -ne 0) {
            throw "STOP: completion branch already exists on origin. Do not overwrite published work."
        }
        Write-Host "REUSE: existing clean completion branch from the previous safe rollback." -ForegroundColor Yellow
    }
    else {
        if ($localBranch.Count -ne 0 -or $remoteBranch.Count -ne 0) {
            throw "STOP: completion branch already exists elsewhere. Do not overwrite it."
        }
    ...
    if ((git -C $wt rev-parse HEAD).Trim() -ne $expectedMain) {
        throw "STOP: completion branch is not based on the audited main commit."
    }
