...
$auditedTayqanMain = "749b14f9b0e22fbe0a74f45091dfc26fd7f2eef5"
$branch = "fix/tayqan-completion-correctness-p1-20260818"
$baseMain = $null

# Main is allowed to advance only when the new commits do not overlap the
# TAYQAN/BOQ correctness implementation being patched here. Dictionary-only
# translation work is allowed because the patch uses structural i18n anchors.
$protectedOverlapPatterns = @(
...
Write-Host "TAYQAN COMPLETION PR 1 — CORRECTNESS CLOSEOUT V3" -ForegroundColor Cyan
...
try {
    ...
    Write-Host "`n=== 2. FETCH + AUDIT CURRENT MAIN FOR REAL OVERLAP ===" -ForegroundColor Cyan
    ...
    git -C $wt merge-base --is-ancestor $auditedTayqanMain origin/main
    if ($LASTEXITCODE -ne 0) {
        throw "STOP: the audited TAYQAN merge is no longer an ancestor of current main. Do not patch automatically."
    }

    $mainChanges = @(
        git -C $wt diff --name-only "$auditedTayqanMain..origin/main"
    ) | ForEach-Object {
        $_.Trim().Replace("\","/")
    } | Where-Object { $_ }

    $realOverlaps = @()
    foreach ($changed in $mainChanges) {
        foreach ($pattern in $protectedOverlapPatterns) {
            if (
                $changed -eq $pattern -or
                ($pattern.EndsWith("/") -and $changed.StartsWith($pattern)) -or
                ($pattern.EndsWith("-") -and $changed.StartsWith($pattern))
            ) {
                $realOverlaps += $changed
                break
            }
        }
    }
    $realOverlaps = @($realOverlaps | Sort-Object -Unique)

    if ($realOverlaps.Count -gt 0) {
        Write-Host "Current main changed protected TAYQAN correctness files:" -ForegroundColor Red
        $realOverlaps | ForEach-Object { Write-Host " - $_" -ForegroundColor Red }
        throw "STOP: main has a real TAYQAN/BOQ overlap. Audit only those changed files before patching."
    }

    $dictionaryChanges = @($mainChanges | Where-Object {
        $_ -eq "src/lib/i18n/dictionaries/en.ts" -or
        $_ -eq "src/lib/i18n/dictionaries/ar.ts"
    })

    if ($main -ne $auditedTayqanMain) {
        Write-Host "SAFE MAIN ADVANCE: audited TAYQAN merge remains in history; no protected correctness overlap." -ForegroundColor Yellow
        Write-Host "New main changed $($mainChanges.Count) file(s); dictionary overlap count: $($dictionaryChanges.Count)." -ForegroundColor Yellow
    }
    else {
        Write-Host "PASS: current main is still the audited TAYQAN merge." -ForegroundColor Green
    }


    Write-Host "`n=== 3. REUSE ONE PR1 BRANCH + FAST-FORWARD TO CURRENT MAIN ===" -ForegroundColor Cyan

    $currentBranch = (git -C $wt branch --show-current).Trim()
    $localBranch = @(git -C $wt branch --list $branch)
    $remoteBranch = @(git -C $wt branch -r --list "origin/$branch")

    if ($remoteBranch.Count -ne 0) {
        throw "STOP: completion branch already exists on origin. Do not overwrite published work."
    }

    if ($currentBranch -eq $branch) {
        git -C $wt merge-base --is-ancestor HEAD origin/main
        if ($LASTEXITCODE -ne 0) {
            throw "STOP: existing PR1 branch contains work not present in main. Do not reset or overwrite it automatically."
        }

        git -C $wt merge --ff-only origin/main
        if ($LASTEXITCODE -ne 0) {
            throw "STOP: could not fast-forward the clean PR1 branch to current main."
        }
        Write-Host "REUSE: existing PR1 branch fast-forwarded safely to current main." -ForegroundColor Yellow
    }
    else {
        if ($localBranch.Count -ne 0) {
            throw "STOP: completion branch already exists in another worktree/context. Do not overwrite it."
        }
    ...
    if ((git -C $wt rev-parse HEAD).Trim() -ne $main) {
        throw "STOP: PR1 branch is not exactly at current audited-compatible main."
    }

    Write-Host "PASS: $branch @ $main" -ForegroundColor Green
...
function replaceRegexOnce(key, regex, newText, label) {
  const text = docs[key];
  const flags = regex.flags.includes("g") ? regex.flags : `${regex.flags}g`;
  const matches = [...text.matchAll(new RegExp(regex.source, flags))];
  if (matches.length !== 1) {
    throw new Error(`${label}: expected exactly one regex anchor, found ${matches.length}`);
  }
  docs[key] = text.replace(regex, newText);
}
...
'@
    ...
    # Windows PowerShell 5.x can corrupt Arabic when a here-string is piped
    # directly to a native executable. Write the Node patch as UTF-8 bytes
    # first, then execute the file so EN/AR dictionary anchors remain exact.
    $nodePatchFile = Join-Path $env:TEMP "tayqan-pr1-correctness-patch.cjs"
    $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
    [IO.File]::WriteAllText($nodePatchFile, $nodePatch, $utf8NoBom)

    & node $nodePatchFile
    $nodeExit = $LASTEXITCODE
    Remove-Item -LiteralPath $nodePatchFile -Force -ErrorAction SilentlyContinue
    ...
    if (-not $committed) {
        ...
        # If a failure occurs after staging, restore the index to HEAD first.
        git -C $wt reset --quiet HEAD -- $allAllowedFiles 2>$null
        ...
        Write-Host "ROLLBACK COMPLETE. Branch remains based on current audited-compatible main." -ForegroundColor Yellow
    }
    else {
