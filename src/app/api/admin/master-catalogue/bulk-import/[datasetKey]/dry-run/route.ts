...
/**
 * CATALOGUE-CLOSE — the production-safe path for activating a validated
 * dataset: the platform owner uploads the CSV through this authenticated
 * admin route (never a production DATABASE_URL handed to anyone). No
 * mutation happens here — see the sibling execute route for that.
 */
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    return apiSuccess(await dataset.dryRun(actor, input));
  } catch (error) {
