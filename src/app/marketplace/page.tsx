  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
        {packages.map((pkg) => (
          <div key={pkg.id} className="flex flex-col rounded-[32px] border border-slate-800 bg-slate-900/80 hover:bg-slate-900 transition-colors p-6 text-slate-300 shadow-sm relative overflow-hidden group">
            
            <div className="flex items-start justify-between gap-2 mb-4">
              <div className="flex items-center gap-4">
                <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-slate-800 text-blue-400 border border-slate-700 shadow-inner group-hover:scale-105 transition-transform">
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 22h14a2 2 0 0 0 2-2V7l-5-5H6a2 2 0 0 0-2 2v4"/><path d="M14 2v4a2 2 0 0 0 2 2h4"/><path d="M3 15h6"/><path d="M3 18h6"/><path d="M14 15h6"/><path d="M14 18h6"/></svg>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-slate-500 font-medium">{pkg.packageType}</p>
                  <h3 className="text-lg font-bold text-white tracking-tight leading-tight mt-1">{pkg.name}</h3>
                </div>
              </div>
              {pkg.isFeatured && <span className="absolute top-6 right-6 rounded-full bg-blue-900/40 border border-blue-800/50 px-2.5 py-1 text-[0.65rem] font-bold uppercase tracking-[0.2em] text-blue-300">Featured</span>}
            </div>
            ...
            <div className="flex items-center justify-between border-t border-slate-800 pt-4 mb-5">
              ...
              <div className="text-right">
                ...
                {pkg.monthlyPrice > 0 && <p className="text-[10px] text-slate-500">{pkg.currency} {pkg.annualPrice}/yr</p>}
              </div>
            </div>
