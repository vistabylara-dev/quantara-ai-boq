...
export default function AIAssistantPanel() {
  ...
  const CAPABILITIES: Array<{ label: string; icon: LucideIcon }> = [
    { label: t("dashboardComponents.aiAssistantPanel.drawingAnalysis"), icon: ScanSearch },
    { label: t("dashboardComponents.aiAssistantPanel.costOptimization"), icon: Gauge },
    { label: t("dashboardComponents.aiAssistantPanel.boqValidation"), icon: FileSearch },
    { label: t("dashboardComponents.aiAssistantPanel.specificationReview"), icon: ShieldCheck },
    { label: t("dashboardComponents.aiAssistantPanel.riskDetection"), icon: AlertTriangle },
  ];
  ...
  return (
    <section className="rounded-[28px] border border-[#009FE3]/30 dark:border-[#21C7F3]/30 bg-gradient-to-br from-[#009FE3]/[0.04] to-transparent dark:from-[#21C7F3]/[0.06] p-6 sm:p-8">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-xl font-semibold text-[#08152E] dark:text-white">{t("dashboardComponents.aiAssistantPanel.title")}</h2>
          <p className="mt-0.5 text-sm text-[#536078] dark:text-[#B8C4D8]">{t("dashboardComponents.aiAssistantPanel.subtitle")}</p>
        </div>
      </div>
        ...
        {CAPABILITIES.map((capability) => (
          ...
          >
            ...
            <span className="rounded-full border border-[#D5E0EC] dark:border-[#20304D] px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-[#7B879C] dark:text-[#7F8DA6]">
              {t("dashboardComponents.aiAssistantPanel.comingSoon")}
            </span>
          </div>
        ))}
      </div>
