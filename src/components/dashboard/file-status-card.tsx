      ...
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-[#7B879C] dark:text-[#7F8DA6]">
            {file.project?.name ?? t("dashboardComponents.shared.unassignedProject")} · {file.extension.toUpperCase()} · {formatFileSize(file.sizeBytes)}
          </p>
        </div>
        <StatusBadge label={file.status} tone={FILE_STATUS_TONE[file.status] ?? "default"} />
      </div>
      <div className="mt-4 flex items-center justify-between">
        <p className="text-xs text-[#7B879C] dark:text-[#7F8DA6]">{t("dashboardComponents.fileStatusCard.uploaded", { date: formatDate(file.createdAt) })}</p>
        <a
        ...
        >
          ...
          {t("dashboardComponents.fileStatusCard.download")}
        </a>
      </div>
    </div>
  );
