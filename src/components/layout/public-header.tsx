      ...
      <div className="container mx-auto px-4 h-16 flex items-center justify-between">
            ...
            return (
              <div key={section.label} className="relative">
                <button
                  ...
                  className="flex items-center gap-1 px-3 py-2 text-sm font-medium text-slate-300 hover:text-white rounded-md hover:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors"
                >
                  {section.label}
                        ...
                        <ul className="space-y-3">
                          {group.items.map((item, iIndex) => (
                            <li key={iIndex}>
                              <Link
                                href={item.href}
                                className="block group/link rounded-lg p-2 -mx-2 hover:bg-slate-900/50 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500"
                              >
                                <div className="flex items-center gap-2 font-medium text-sm text-slate-100 group-hover/link:text-blue-400">
                                  <span>{item.label}</span>
                                  {item.status ? (
                                    <span className="rounded-full bg-blue-950 px-2 py-0.5 text-[10px] font-bold text-blue-200">
                                      {item.status}
                                    </span>
                                  ) : null}
                                </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="hidden lg:flex items-center gap-4">
            <Link href="/login" className="text-sm font-medium text-slate-300 hover:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-md px-2 py-1">
        ...
        >
          <div className="px-4 py-6 space-y-2">
            {navigation.map((section) => {
              ...
              return (
                <div key={section.label} className="border-b border-slate-800/60 pb-2">
                  <button
                    id={buttonId}
                    aria-expanded={isOpen}
                    aria-controls={panelId}
                          ...
                          <ul className="space-y-3">
                            {group.items.map((item, iIndex) => (
                              <li key={iIndex}>
                                <Link
                                  href={item.href}
                                  className="block text-base text-slate-300 hover:text-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-md py-1"
                                >
                                  <span>{item.label}</span>
                                  {item.status ? (
            ...
            <div className="pt-8 space-y-4">
              <Link href="/login" className="block w-full text-center py-3 text-base font-medium text-white border border-slate-800 rounded-lg hover:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
                {t("publicSite.header.signIn")}
              </Link>
