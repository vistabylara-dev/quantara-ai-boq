...
const statusColors: Record<PublicCapabilityStatus, string> = {
  ...
  NOT_AVAILABLE: "bg-slate-700",
};

const statusBadgeColors: Record<PublicCapabilityStatus, string> = {
  AVAILABLE: "bg-green-900/30 text-green-400",
  CONTROLLED_ACCESS: "bg-blue-900/30 text-blue-400",
  LIMITED: "bg-amber-900/30 text-amber-400",
  NOT_AVAILABLE: "bg-slate-800 text-slate-300",
};
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#030508] text-slate-100">
      ...
      <div className="flex-1 pb-24">
        {/* Breadcrumb */}
        <div className="border-b border-slate-800/60 bg-[#030508] px-4 py-4">
        ...
        <div className="container mx-auto max-w-4xl px-4 pt-16 pb-12">
          ...
          <p className="text-xl md:text-2xl text-slate-300 leading-relaxed font-medium mb-16">
          ...
          <div className="space-y-16">
              ...
              {content.audience.items && (
                <ul className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-6">
                  {content.audience.items.map((item, idx) => (
                    <li key={idx} className="flex items-start gap-3 text-slate-300 bg-slate-900/50 p-4 rounded-xl border border-slate-800">
            ...
            <section>
              ...
              <div className="grid gap-6">
                {content.relevantFeatures.map((feature) => {
                  ...
                  return (
                    <div key={feature.name} className="p-6 bg-slate-900/50 rounded-2xl border border-slate-800 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
                      <div>
                        <h3 className="text-lg font-bold mb-2 text-white">{feature.name}</h3>
                        <p className="text-slate-400">{feature.description}</p>
                      </div>
                      <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-800/50 rounded-full text-sm font-medium border border-slate-700 shrink-0">
            ...
            <section className="bg-blue-900/10 rounded-3xl p-8 border border-blue-900/30">
              <h2 className="text-2xl font-bold mb-4 text-white">{content.workflowExample.heading}</h2>
              <p className="text-lg text-slate-400 mb-8">{content.workflowExample.introduction}</p>
              
              <div className="space-y-6 relative before:absolute before:inset-0 before:ms-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-blue-800 before:to-transparent">
                {content.workflowExample.steps.map((step, idx) => (
                  <div key={idx} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                    <div className="flex items-center justify-center w-10 h-10 rounded-full border-4 border-[#030508] bg-blue-900 text-blue-400 shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 shadow-sm z-10">
                      <span className="font-bold text-sm">{idx + 1}</span>
                    </div>
                    <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-5 rounded-2xl bg-slate-900 border border-slate-800 shadow-sm">
              ...
              <section>
                ...
                <div className="space-y-4">
                  {content.supportedInputs.map((input) => {
                    ...
                    return (
                      <div key={input.name} className="p-4 bg-slate-900/50 rounded-xl border border-slate-800 shadow-sm">
                        ...
                        {boundary && (
                          <p className="text-xs text-amber-400 mt-2 font-medium">{t("publicLanding.note")} {boundary}</p>
                        )}
                      </div>
                    );
                  })}
              ...
              <section>
                ...
                <div className="space-y-4">
                  {content.supportedOutputs.map((output) => {
                    ...
                    return (
                      <div key={output.name} className="p-4 bg-slate-900/50 rounded-xl border border-slate-800 shadow-sm">
                        <div className="flex items-center justify-between mb-2">
            ...
            <section className="bg-slate-900 p-8 rounded-2xl border border-slate-800">
              ...
              <ul className="space-y-3">
                {content.limitations.map((limit, idx) => (
                  <li key={idx} className="flex items-start gap-3 text-slate-300">
                    <span className="w-1.5 h-1.5 rounded-full bg-slate-600 shrink-0 mt-2.5" />
            ...
            <section className="rounded-e-xl border-s-4 border-amber-500 bg-amber-900/20 p-6">
              <h3 className="font-bold text-amber-300 mb-2">{t("publicLanding.professionalDisclaimer")}</h3>
            ...
            <section>
              ...
              <div className="max-w-3xl mx-auto space-y-4">
                {content.faqs.map((faq) => (
                  <details key={faq.question} className="group overflow-hidden rounded-xl border border-slate-800 bg-slate-950">
                    <summary className="flex w-full cursor-pointer list-none items-center justify-between px-6 py-4 text-start focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-inset [&::-webkit-details-marker]:hidden">
                      <span className="pe-4 font-semibold text-slate-100">{faq.question}</span>
            ...
            <section className="border-t border-slate-800 pt-16">
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {content.relatedPages.map((page, idx) => (
                  <Link key={idx} href={page.href} className="group p-6 rounded-2xl bg-slate-900/50 border border-slate-800 hover:border-blue-500 transition-colors block">
                    <h3 className="font-bold text-white mb-2 group-hover:text-blue-400 transition-colors">{page.label}</h3>
