...
export default function Sidebar() {
  ...
  return (
    <aside
      className={`hidden xl:flex xl:flex-col xl:gap-6 xl:border-e xl:border-white/10 xl:py-8 bg-black/40 backdrop-blur-xl text-slate-300 transition-[width] relative z-20 cyber-border ${
    ...
    >
      <div className="flex items-center justify-between gap-2 border-b border-white/10 pb-6 mb-2">
        {!collapsed && (
          <div className="space-y-1">
            <p className="terminal-text text-[9px] uppercase tracking-[0.4em] text-[#00F0FF]">{t("navigation.systemActive")}</p>
            <h2 className="text-xl font-bold text-white tracking-widest uppercase">{t("common.appName")}</h2>
          </div>
        )}
        <button
          type="button"
          onClick={() => setCollapsed((value) => !value)}
          aria-label={collapsed ? t("a11y.expandNavigation") : t("a11y.collapseNavigation")}
        ...
        >
          {/* Direction-sensitive: this chevron points toward the collapsed/expanded edge, which flips under RTL. */}
          {collapsed ? (
      ...
      <nav className="flex flex-1 flex-col gap-6 overflow-y-auto" aria-label={t("navigation.primaryNav")}>
        {GROUPS.map((group) => {
          ...
          return (
            <div key={group.labelKey} className="flex flex-col gap-2">
              {!collapsed && (
                <p className="px-3 terminal-text text-[10px] uppercase tracking-[0.3em] text-slate-600 mb-2">{t(group.labelKey)}</p>
              )}
              {groupItems.map((item) => {
                ...
                return (
                  ...
                  >
                    {isActive && (
                      <span className="absolute start-0 top-0 bottom-0 w-[2px] bg-[#00F0FF] shadow-[0_0_10px_#00F0FF]" aria-hidden="true" />
                    )}
      ...
      {!collapsed && (
        <div className="relative overflow-hidden border border-[#00F0FF]/20 bg-[#00F0FF]/5 p-5 text-sm mt-4 cyber-panel">
          <span className="absolute -end-6 -top-6 h-16 w-16 rounded-full bg-[#00F0FF]/10 animate-pulse" aria-hidden="true" />
          <p className="font-bold text-[#00F0FF] uppercase tracking-widest text-[11px] mb-2">{t("navigation.engineStatus")}</p>
          <div className="flex items-center gap-2 text-xs text-slate-400 terminal-text">
            <span className="w-2 h-2 rounded-full bg-[#00F0FF] animate-pulse"></span>
            {t("navigation.allSystemsNominal")}
          </div>
        </div>
      )}
    </aside>
