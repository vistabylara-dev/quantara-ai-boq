...
const POLICIES: Record<AutonomousIndustryEngineId, AutonomousIndustryPolicy> = {
  construction: policy("construction", [
    ...
    rule("gross-floor-area", "area-schedules", QuantityCalculationType.FLOOR_AREA, "m2", "Explicit gross floor area", ["grossFloorArea"]),
    rule("net-floor-area", "area-schedules", QuantityCalculationType.FLOOR_AREA, "m2", "Explicit net floor area", ["netFloorArea"]),
    rule("room-schedule-count", "space-schedules", QuantityCalculationType.COUNT, "nos", "Reconciled room schedule count", ["roomCount"]),
    rule("construction-floor-finish-area", "floor-finishes", QuantityCalculationType.FLOOR_AREA, "m2", "Scheduled floor finish area", ["flooringArea"]),
    rule("construction-ceiling-area", "ceilings", QuantityCalculationType.CEILING_AREA, "m2", "Scheduled ceiling area", ["ceilingArea"]),
    rule("construction-wall-finish-area", "wall-finishes", QuantityCalculationType.WALL_AREA, "m2", "Scheduled wall finish area", ["wallFinishArea"]),
    rule("door-window-count", "openings", QuantityCalculationType.COUNT, "nos", "Reconciled door and window schedule count", ["openingCount"]),
    rule("construction-partition-area", "partitions", QuantityCalculationType.PARTITION_AREA, "m2", "Measured partition area", ["partitionArea"]),
    rule("external-works-area", "external-works", QuantityCalculationType.FLOOR_AREA, "m2", "Measured external works area", ["externalWorksArea"]),
  ]),
