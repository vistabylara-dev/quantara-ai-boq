...
export type ImportEntityToBoqInput = {
  ...
  category: string;
  description: string;
  /** Explicit BOQ specification reviewed by the professional before HTTP import. */
...
export async function importExtractedEntityToBoq(
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const preparedItem = await prepareBOQItemCreation(actor.companyId, input.sectionId, {
      ...
      // HTTP callers supply an explicit, visible professional-reviewed
      // specification. Direct legacy service callers retain their previous
      // sourceText fallback for backward compatibility.
      specification: input.specification ?? entity.sourceText ?? "",
